...
An R package for robust tests of overall fit and nested model comparisons in structural equation models. It works with `lavaan` and implements the penalized eigenvalue block averaging and penalized regression *p* values of Foldnes, Moss, and Grønneberg (2025), along with their extension to nested model comparison in Foldnes, Grønneberg, and Moss (2026).
...
```{r prepare, echo = TRUE, eval = TRUE, results = "hide"}
library("semTests")
model <- "visual  =~ x1 + x2 + x3
          textual =~ x4 + x5 + x6
          speed   =~ x7 + x8 + x9"
...
```{r nested, echo = TRUE, eval = TRUE, results = "hide"}
...
Savalei (2010). The [FIML vignette](vignettes/fiml-missing-data.Rmd) explains
this choice and shows the other options.

```{r fiml, echo = TRUE, eval = TRUE, results = "hide"}
...
```{r categorical, echo = TRUE, eval = TRUE, results = "hide"}
