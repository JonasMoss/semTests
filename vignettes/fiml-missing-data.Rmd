...
defined for a different data setting and are unavailable here.
...
dependent on `x9`, which remains observed. This gives us a reproducible
MAR-like mechanism for the example. It says nothing about the missingness
process in the original data.
...
Two related controls appear in this workflow:
...
weight convention and the model information selected in the fit.
`information = "expected"` controls only the model-information part of that
construction.
...
develop the consequences for SEM. The switch can matter in finite samples, so
we also checked it in a focused computational study.
...
estimate and standard-error tails. We therefore omit arithmetic mean SE/SD
ratios. Coverage retains converged replications with finite standard errors,
...
parameter-level standard-error comparisons. The maximum absolute difference
was \(5.81\times10^{-7}\).

The outcome studied here is parameter standard-error coverage. Null
calibration of the `semTests` goodness-of-fit *p*-values has its own checks
through deterministic spectrum parity and the seeded null simulation under
`tools/`. The coverage results favor observed information as the default
building block, especially when a nested comparison uses the full observed
model information. The `"lavaan"` convention remains available whenever the
goal is to reproduce lavaan's inspected spectrum.
...
can pull them apart. `semTests` keeps the convention visible in both the
function call and the result provenance.
