...
so we gave it a fairly serious computational check.
...
Each of the 48 cells used 200,000 replications, giving 9.6 million fits in all.
...
For a misspecified model, centered coverage uses the simulation's average
estimate as the target. That lets us study interval calibration without
pretending that a misspecified parameter is the original population value.

The full observed Hessian with sandwich covariance came closest to 95% overall.
Its mean absolute coverage error was 0.821 percentage points, with mean
coverage of 94.179% in the correctly specified cells. Under a correct normal
model, the two full-Hessian versions were tied at about 94.7%. Under \(t_5\)
data, model-based covariance hovered near 80%, while the full-Hessian sandwich
reached 93.64%. The same method also did best in the misspecified cells shown
in the table.

A few converged fits had Heywood estimates or nearly singular information
matrices. Those fits create wild estimate and standard-error tails, so we do
not report arithmetic mean SE/SD ratios. Coverage keeps converged replications
with finite standard errors. The worst information-inversion failure rate in
any cell was 0.0105%.

We also compared the simulation calculations with lavaan in all 576
parameter-level checks. Every check agreed, and the largest absolute difference
was \(5.81\times10^{-7}\).

This study is about parameter interval coverage. It does not directly test the
null calibration of `semTests` goodness-of-fit p-values, which has separate
deterministic and seeded simulation checks under `tools/`. The result still
gives a good practical reason to use observed information as the default
building block. The `"lavaan"` convention remains handy when reproduction of
lavaan's inspected spectrum is the goal.
...
They can be close in friendly settings. Missingness and non-normality can pull
them apart. `semTests` keeps the convention visible in both the function call
...
The delta default is the forgiving choice when equivalent models have different
parameter tables. Use exact when the hypothesis is naturally written as a
