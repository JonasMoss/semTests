...
so we checked it with a large simulation.[^sim]
...
overall, and its advantage is largest under heavy tails and misspecification.
Under \(t_5\) data the model-based intervals drop to about 80%, while the
full-Hessian sandwich stays above 93%. The lavaan calculations agreed with ours
throughout.[^validate]
...
gives a practical reason to use observed information as the default building
block. The `"lavaan"` convention remains available when the goal is to
reproduce lavaan's inspected spectrum.

[^sim]: The 48 cells used 200,000 replications each, 9.6 million fits in all. A few converged fits had Heywood estimates or nearly singular information matrices, which produce wild standard-error tails. Coverage keeps only converged replications with finite standard errors, and we do not report mean SE/SD ratios. The worst information-inversion failure rate in any cell was 0.0105%.

[^validate]: Across all 576 parameter-level checks the largest absolute difference from lavaan was \(5.81\times10^{-7}\).
