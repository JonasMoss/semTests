...
.imhof_one <- function(q, lambda, rel.tol = 1e-10, subdivisions = 10000L) {
  res <- tryCatch(
    stats::integrate(.imhof_integrand, 0, Inf,
      q = q, lambda = lambda,
  ...
  if (is.null(res)) {
    return(list(prob = NA_real_, ok = FALSE))
  }
...
.saddle_root <- function(q, lambda) {
  ...
  while (Kp(lo) > q && it < 300L) {
    lo <- if (is.finite(lb)) lb + (lo - lb) * 0.5 else lo * 2
    it <- it + 1L
  }
  it <- 0L
  while (Kp(hi) < q && it < 300L) {
    hi <- if (is.finite(ub)) ub - (ub - hi) * 0.5 else hi * 2
    it <- it + 1L
  }
  if (Kp(lo) > q || Kp(hi) < q) {
    return(NA_real_)
  }
  stats::uniroot(function(t) Kp(t) - q,
    lower = lo, upper = hi,
...
.ruben_one <- function(q, lambda, eps = 1e-12, maxit = 5000L) {
  ...
  repeat {
    ...
    acoef <- c(acoef, ak)
    gcoef <- c(gcoef, g(k))
    surv <- surv + ak * stats::pchisq(q / beta,
      df = m + 2L * k,
      lower.tail = FALSE
    )
    asum <- asum + ak
